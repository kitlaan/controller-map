...
import IconPicker, { PickerItem } from './IconPicker.vue';
...
function checkDisabled() {
  return props.mode === 'add' && props.modelValue.id === ''
}
</script>
...
<template>
  <div class="flex flex-row gap-2 rounded-md border border-solid border-gray-400 p-1 mb-1">
    ...
    <IconPicker v-model="modelValue.id" :items="items" :dropUp="mode === 'add'" style="min-width: 40px" />
    <IconPicker v-model="modelValue.modifiers" :items="items" :dropUp="mode === 'add'" style="min-width: calc(40px * 3)" placeholder="modifiers" />
